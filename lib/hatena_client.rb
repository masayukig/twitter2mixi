...
class HatenaClient
  def initialize
    ...
    # ログ出力
    WWW::Mechanize.log = Logger.new(File.expand_path(File.dirname(__FILE__)) + '/../log/mechanize_hatena.log')
...
#    @login_flg = page.header['content-type'] != 'text/html; charset=ISO-8859-1'
  ...
  # 書き込む際に、" #{twitter_url}"という文字列を後ろに付加します。
  def write_haikus timeline, twitter_url
    return nil if @login_flg == false
    ...
    timeline.reverse_each {|text|
      count += 1 if write_haiku(text, twitter_url) != nil
      sleep 0.5
    }
