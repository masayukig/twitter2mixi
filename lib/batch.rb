...
puts "(Userid=#{user.id})client.user[0]['created_at']=#{client.user[0]['created_at']}"
        # 会員情報の更新
        ...
        client.user.each { |status|
          if "#{status.class}" == 'Hash'
              ...
              puts "(Userid=#{user.id})status['created_at']=#{status['created_at']}"
              puts "(Userid=#{user.id})status['created_at']=#{status['text'].tosjis}"
